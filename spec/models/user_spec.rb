...
RSpec.describe User, type: :model do
  ...
  describe "#words_count" do
    before do
      ...
      m = Message.create!(user: @user, title: "global disaster", content: "cause many damage")
      Comment.create!(user: @user, content: "typhoon is terrible", message: m)
    end

    it "加總該使用者的所有 Mesasge 和 Comment 的總字數" do
      # TODO: 測試 words_count 方法
      expect(@user.words_count).to eq(8)
    end
  end
