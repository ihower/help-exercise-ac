...
RSpec.describe User, type: :model do
  ...
  describe "#words_count" do
    before do
      ...
      another_user = User.create!( :email => "tony@example.org", :password => "12345678")
      m1 = Message.create!( :user => @user, :title =>"title",:content =>"content" )
      Comment.create!( :user => another_user, :message => m1,:content =>"content" )

      m2 = Message.create!( :user => another_user, :title =>"title",:content =>"content" )
    ...
    it "加總該使用者的所有 Mesasge 和 Comment 的總字數" do
      ...
      expect( @user.words_count ).to eq(4)
