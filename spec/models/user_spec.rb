...
RSpec.describe User, type: :model do
  ...
  describe "#words_count" do
    before do
      ...
      @other_user = User.create!( :email => "ooxx@example.org", :password => "12345678")
      m1 = Message.create!( :user => @other_user, :title => "1", :content => "22" )
      Comment.create!( :user => @other_user, :message_id => m1, :content => "66 66 66"  )

      m2 = Message.create!( :user => @user, :title => "3 3 3", :content => "4444" )
      Comment.create!( :user => @user, :message_id => m2, :content => "55555" )
      Comment.create!( :user => @user, :message_id => m2, :content => "2 2" )
    end

    it "加總該使用者的所有 Mesasge 和 Comment 的總字數" do
      # TODO: 測試 words_count 方法
      expect( @user.words_count ).to eq(7)
      expect( @other_user.words_count ).to eq(5)
    end
  end
