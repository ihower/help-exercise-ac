...
RSpec.describe User, type: :model do
  ...
  describe "#words_count" do
    before do
      ...
      other_user = User.create!( :email => "ooxx@example.org", :password => "12345678")
      m1 = Message.create!( :user => other_user, :title=>"i hate surfing", :content=>"surfing is terrible" )
      Comment.create!( :user => other_user, :message => m1, :content=>"wow" )

      m2 = Message.create!( :user => @user, :title=>"This is good fruit", :content=>"the watermelon is sweet and has so much water" )
      Comment.create!( :user => @user, :message => m2, :content=>"really i want one, too")
      Comment.create!( :user => @user, :message => m2, :content=>"oh i want too" )
    end
