class MessagesController < ApplicationController
  ...
  def index
    ...
    if params[:status] == "pending"
      ...
      @messages = @messages.includes(:user).where( :status => "pending" )
    elsif params[:status] == "completed"
      # TODO: @messages = @messages.completed
      @messages = @messages.includes(:user).where( :status => "completed" )
    end

    if params[:days]
      # TODO: @messages = @messages.within_days(params[:days].to_i)
      @messages = @messages.includes(:user).where( ["created_at >= ?", Time.now - params[:days].to_i.days ] )
    end
  end
